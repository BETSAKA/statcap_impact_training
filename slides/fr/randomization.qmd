...
subtitle: "Atelier Statcap : Evaluation d'impact sur données spatiales"
author: "Florent Bédécarrats & Thomas Thivillon"
date: "Juillet 2025"
format: 
  clean-revealjs:
    slide-number: true
...
flowchart TD
    A["Population<br/>éligible"] --> AB(Faisabilité)
    AB --> B["Échantillon<br/>étudié"]
    B --> C("Assignation<br/>aléatoire")
    C --> D["Groupe de<br/>traitement"]
    C --> E["Groupe de<br/>contrôle"]

    style A fill:#d3d3d3,color:#000000
    style B fill:#d3d3d3,color:#000000
    style D fill:#3cb371,color:#000000
    style E fill:#ff6347,color:#000000
```
...
- L'objectif de la régression linéaire simple est de modéliser la relation entre une variable dépendante $y$ et une variable indépendante $x$.
- On fait l'hypothèse que cette relation est linéaire.
- On recherche la ligne qui va minimiser la distance verticale entre chacun des points de données $(x_i, y_i)$ et elle_même.
...
set.seed(123)
x <- seq(1, 20)
y <- 2 + 0.5 * x + rnorm(20, sd = 2)
df <- data.frame(x = x, y = y)

# Ajuster un modèle linéaire
mod <- lm(y ~ x, data = df)
df$y_pred <- predict(mod)

# Sans résidus
p1 <- ggplot(df, aes(x = x, y = y)) +
  geom_point(size = 2, color = "blue") +
  labs(title = "(a) Nuage de points", x = "x", y = "y") +
...
p2 <- ggplot(df, aes(x = x, y = y)) +
  ...
  labs(title = "(b) Nuage et ligne de régression", subtitle = "(résiduels en pointillés)", x = "x", y = "y") +
  theme_minimal(base_size = 14)

```

::: {#fig-regression .center}
```{r}
p1 + p2
```
:::
...
- On cherche à estimer les coefficients $a$ et $b$ de la droite : 

$$ \hat{y} = a + bx $$
- Cette droite est notre estimateur de la relation entre $x$ et $y$ dans la population : 
...
- On peut montrer que le coefficient $b$ est donné par la formule :
...
- On peut ensuite utiliser le fait que la droite de régression passe nécessairement par le point moyen $(\bar{x}, \bar{y})$ pour en déduire la valeur de $a$ :
...
```{r}
# Recalculer les éléments nécessaires
x_bar <- mean(df$x)
y_bar <- mean(df$y)
intercept <- coef(mod)[1]

# Créer le graphique enrichi
p2 <- ggplot(df, aes(x = x, y = y)) +
  geom_point(size = 2, color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  ...
  geom_text(aes(x = x_bar, y = y_bar, label = "(x̄, ȳ)"),
            vjust = -1.2, hjust = 0.5, color = "red", size = 5) +

  # Intercept
  geom_point(aes(x = 0, y = intercept),
             color = "darkgreen", size = 4, shape = 17) +
  geom_text(aes(x = 0, y = intercept, label = "a"),
            vjust = -1.2, hjust = -0.3, color = "darkgreen", size = 5) +
  
  labs(title = "(b) Nuage et droite de régression",
       subtitle = "Résidus en pointillés, centre de gravité et intercept visibles",
       x = "x", y = "y") +
  theme_minimal(base_size = 14)
...
On modélise la relation entre la distance à la ville la plus proche et le taux de déforestation annuel de 4 aires protégées. 
...
# Créer les données
parcs <- data.frame(
  nom = c("Aire 1", "Aire 2", "Aire 3", "Aire 4"),
  distance_ville = c(20, 50, 80, 100),    # km
  deforestation = c(3.5, 2.8, 1.9, 1.2)   # en %
)

knitr::kable(parcs, caption = "Données des 4 aires protégées")
...
$$
\bar{x} = \frac{20 + 50 + 80 + 100}{4} = \frac{250}{4} = 62.5
$$

$$
\bar{y} = \frac{3.5 + 2.8 + 1.9 + 1.2}{4} = \frac{9.4}{4} = 2.35
$$
...
|----------|-----------|---------------------|---------------------|--------------------------------------|-------------------------|
| 20       | 3.5       | -42.5               | 1.15                | -48.875                              | 1806.25                 |
| 50       | 2.8       | -12.5               | 0.45                | -5.625                               | 156.25                  |
| 80       | 1.9       | 17.5                | -0.45               | -7.875                               | 306.25                  |
| 100      | 1.2       | 37.5                | -1.15               | -43.125                              | 1406.25                 |

$$\
\sum (x_i - \bar{x})(y_i - \bar{y}) = -105.5 \quad ; \quad \sum (x_i - \bar{x})^2 = 3675
$$
...
$$\
b = \frac{-105.5}{3675} ≈ -0.0287
$$


$$\
a = \bar{y} - b \cdot \bar{x} = 2.35 - (-0.0287 \cdot 62.5) = 2.35 + 1.79375 ≈ 4.144
$$
...
$$
\hat{y} = 4.14 - 0.029 \cdot x
$$

```{r}

# Modèle linéaire
mod <- lm(deforestation ~ distance_ville, data = parcs)
...
ggplot(parcs, aes(x = distance_ville, y = deforestation)) +
  geom_point(size = 3, color = "forestgreen") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(
    x = "Distance à la ville la plus proche (km)",
    y = "Taux de déforestation annuel (%)"
  ) +
