...
AP_RCT <- AP_Vahatra %>%
  ...
  mutate(Groupe = ifelse(year(date_creation) < 2015, "Traitement", "Contrôle"),
         ...
         `Couvert forestier en 2000 (ha)` = `treecover_area_2000-01-01_treecover_ha`,
  ...
  rename(`Altitude moyenne (m)` = `elevation_2000-02-01_elevation_mean_m`,
         ...
         `Temps de trajet (minutes)` = `traveltime_2015-01-01_5k_110mio_traveltime_mean_minutes`)
...
- Régressez à nouveau le taux de déforestation moyen entre 2000 et 2014 sur la variable de traitement en ajoutant le taux de déforestation en 2000 comme variable de contrôle (vous ajoutez un deuxième régresseur en plus de 'Groupe').
