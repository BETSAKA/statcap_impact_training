...
- Une base d’individus avec leur commune de résidence
- Une base de communes avec des indicateurs agrégés (ex: pauvreté, déforestation)

**Objectif** : faites une jointure pour enrichir la base individuelle avec les caractéristiques de la commune.

```{r}
# Base individuelle
individus <- data.frame(ID = 1:4,
                        commune = c("A", "B", "A", "C"),
                        revenu = c(200, 250, 180, 220))

# Base agrégée
communes <- data.frame(commune = c("A", "B", "C"),
                       deforestation = c(0.2, 0.1, 0.3))
```

À vous de jouer : utilisez `left_join()`.
